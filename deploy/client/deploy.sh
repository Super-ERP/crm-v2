...
parse_environment() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    case "$key" in
      ...
      BACKUP_REMOTE_TARGET) BACKUP_REMOTE_TARGET=$value ;;
      BACKUP_REQUIRE_OFFSITE) BACKUP_REQUIRE_OFFSITE=$value ;;
      BACKUP_AGE_RECIPIENT) BACKUP_AGE_RECIPIENT=$value ;;
      BACKUP_INTERVAL_SECONDS) BACKUP_INTERVAL_SECONDS=$value ;;
...
compose() (
  ...
  export BACKUP_RSYNC_TARGET BACKUP_REMOTE_TARGET BACKUP_REQUIRE_OFFSITE
  export BACKUP_AGE_RECIPIENT BACKUP_INTERVAL_SECONDS BACKUP_RETENTION_DAYS
  export BACKUP_MAX_AGE_SECONDS GATEWAY_HOST_PORT DB_HOST_PORT
...
unset BACKUP_RSYNC_TARGET BACKUP_REMOTE_TARGET BACKUP_REQUIRE_OFFSITE
unset BACKUP_AGE_RECIPIENT BACKUP_INTERVAL_SECONDS BACKUP_RETENTION_DAYS
unset BACKUP_EVIDENCE_FILE BACKUP_EVIDENCE_SIGNATURE_FILE
...
BACKUP_RSYNC_TARGET= BACKUP_REMOTE_TARGET= BACKUP_REQUIRE_OFFSITE=
BACKUP_AGE_RECIPIENT= BACKUP_INTERVAL_SECONDS= BACKUP_RETENTION_DAYS=
BACKUP_EVIDENCE_FILE= BACKUP_EVIDENCE_SIGNATURE_FILE=
...
required BACKUP_AGE_RECIPIENT "$BACKUP_AGE_RECIPIENT"
...
printf '%s\n' "$BACKUP_AGE_RECIPIENT" | grep -Eq '^age1[0-9a-z]+$' ||
